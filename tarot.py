...
main_kb = ReplyKeyboardMarkup([
    [KeyboardButton("🔮 Tarot မေးမည်"), KeyboardButton("❤️ ဖူးစာစစ်မည်")],
    [KeyboardButton("📤 Share မည်"), KeyboardButton("✍️ အကြံပြုမည်")],
...
def init_db():
    ...
    try:
        ...
        # (၂) is_banned column မရှိသေးလျှင် ထပ်ထည့်ခြင်း
        try:
            with conn.cursor() as c:
                c.execute("ALTER TABLE users ADD COLUMN is_banned BOOLEAN DEFAULT FALSE")
            conn.commit()
        except Exception:
            conn.rollback() # 👈 Column ရှိပြီးသားဖြစ်၍ Error တက်လျှင် Rollback လုပ်ပြီး ကျော်သွားမည်

        # (၃) Feedbacks Table တည်ဆောက်ခြင်း
        with conn.cursor() as c:
...
# 💡 tarot_init နှင့် share_logic တို့၏ အစတွင် စစ်ဆေးရန် အောက်ပါစာကြောင်း ထည့်ပေးပါ 💡
# (ဥပမာ tarot_init ၏ အစတွင်)
...
async def cancel_tarot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inline button မှ 'မမေးတော့ပါ' နှိပ်လျှင် ဖျက်သိမ်းရန်"""
    query = update.callback_query
    await query.answer()

    cancel_text = (
        "❌ မေးမြန်းခြင်းကို ဖျက်သိမ်းလိုက်ပါပြီရှင်။\n\n"
        "လူကြီးမင်း အဆင်သင့်ဖြစ်တဲ့အခါမှ '🔮 Tarot မေးမည်' ကိုနှိပ်ပြီး "
        "အချိန်မရွေး ပြန်လည်မေးမြန်းနိုင်ပါတယ်ရှင်။ Tarot BayDin မှ အမြဲကြိုဆိုနေပါတယ်။ ✨"
    )
...
async def tarot_init(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """Tarot မေးမည်ကို နှိပ်လိုက်လျှင် Credit စစ်ဆေးပြီး မွေးနေ့ကို အမြဲတမ်းပြန်တောင်းခြင်း"""
    user_id = update.effective_user.id
    
    conn = db_mgr.get_conn()
    try:
        with conn.cursor() as c:
            # Credit ရှိမရှိ တစ်ခုတည်းကိုသာ စစ်ဆေးခြင်း
            c.execute("SELECT credits FROM users WHERE user_id = %s", (user_id,))
            res = c.fetchone()
        
        # ၁။ Credit ရှိမရှိ စစ်ဆေးခြင်း (Admin ဆိုလျှင် ကျော်မည်)
        if user_id != ADMIN_ID:
            if not res or res[0] is None or res[0] <= 0:
                await update.message.reply_text(
                    "❌ လူကြီးမင်းမှာ Credit မလုံလောက်တော့ပါရှင်။ \n📢 Share မည် ကိုနှိပ်ပြီး Credit ထပ်ယူနိုင်ပါတယ်ရှင်။",
                    reply_markup=main_kb # Global kb ကို သုံးထားပါသည်
                )
                return ConversationHandler.END
        ...
        logger.error(f"Error in tarot_init: {e}")
        await update.message.reply_text("❌ စနစ်ပိုင်းဆိုင်ရာ အမှားအယွင်းရှိနေပါသည်။ ခေတ္တစောင့်ပေးပါရှင်။")
        return ConversationHandler.END
    finally:
        db_mgr.put_conn(conn)
        
async def love_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """❤️ ဖူးစာစစ်မည် ခလုတ် နှိပ်လျှင် စတင်မည့်နေရာ"""
...
async def tarot_love_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၁။ Validation စစ်ဆေးခြင်း (၁ မှ ၇၈ အတွင်း မဟုတ်လျှင်)
    if not text.isdigit() or not (1 <= int(text) <= 78):
        await update.message.reply_text("❌ မှားယွင်းနေပါသည်။ ကျေးဇူးပြု၍ ၁ မှ ၇၈ အတွင်းရှိ ဂဏန်းတစ်ခုကိုသာ ရိုက်ထည့်ပေးပါရှင်", 
                                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ မမေးတော့ပါ", callback_data="love_cancel")]]))
        return TAROT_LOVE_INPUT

    # ၂။ Credit စစ်ဆေးခြင်း
    conn = db_mgr.get_conn()
    c = conn.cursor()
    c.execute("SELECT credits FROM users WHERE user_id = %s", (user.id,))
    res = c.fetchone()
    if not res or res[0] <= 0:
        db_mgr.put_conn(conn)
        keyboard = [[InlineKeyboardButton("📤 Share မည်", callback_data="share_bot")]]
        await update.message.reply_text("❌ Credit မလုံလောက်ပါ။\nShare ခလုတ်ကိုနှိပ်၍ Credit အခမဲ့ ရယူပါရှင်။", reply_markup=InlineKeyboardMarkup(keyboard))
        return ConversationHandler.END
        
    # ၃။ Credit ၁ ခု ဖြတ်ခြင်း
    c.execute("UPDATE users SET credits = credits - 1 WHERE user_id = %s", (user.id,))
    conn.commit()
    db_mgr.put_conn(conn)
...
# ၅။ Random ကဒ်ရွေးချယ်ခြင်း (Data ရှိမရှိ အရင်စစ်မည်)
    if TAROT_LOVE_DATA:
        ...
        card = {
            ...
            "image_url": "[https://upload.wikimedia.org/wikipedia/en/d/de/RWS_Tarot_06_Lovers.jpg](https://upload.wikimedia.org/wikipedia/en/d/de/RWS_Tarot_06_Lovers.jpg)", 
    ...
    caption = f"✨ {user.full_name} ရရှိသောကဒ်"
...
async def zodiac_confirm_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # '✅ မှန်ကန်ပါသည်' ကို နှိပ်ပါက Credit စစ်မည်
    conn = db_mgr.get_conn()
    c = conn.cursor()
    c.execute("SELECT credits FROM users WHERE user_id = %s", (user.id,))
    res = c.fetchone()
    if not res or res[0] <= 0:
        db_mgr.put_conn(conn)
        keyboard = [[InlineKeyboardButton("📤 Share မည်", callback_data="share_bot")]]
        await query.edit_message_text("❌ Credit မလုံလောက်ပါ။\nShare ခလုတ်ကိုနှိပ်၍ Credit အခမဲ့ ရယူပါရှင်။", reply_markup=InlineKeyboardMarkup(keyboard))
        return ConversationHandler.END
...
async def process_reading(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၇။ Credit နှုတ်ခြင်း (Admin မဟုတ်လျှင်)
    if user_id != ADMIN_ID:
        conn = db_mgr.get_conn(); c = conn.cursor()
...
async def handle_feedback_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "give_review":
        await query.edit_message_text(
            "ကျေးဇူးတင်ပါတယ်ရှင်။ လူကြီးမင်းရဲ့ Review ကို အောက်မှာ စာသားအတိုင်း ရိုက်ပို့ပေးနိုင်ပါတယ်ရှင်။"
        )
        return WAIT_REVIEW_TEXT
    else:
        # ၁။ "Review ပေးမလား" ဟု မေးထားသော Message အဟောင်းကို ဖျက်ပစ်မည်
        try:
            await query.message.delete()
        except:
            pass
        
# 💡 ၁။ handle_feedback_choice Function ထဲက Keyboard နေရာကို အောက်ပါအတိုင်း ပြင်ပါ
# (Review မပေးဘူး နှိပ်လိုက်ရင် ပေါ်လာမယ့် ခလုတ်များ)


        
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Tarot Bot ကို အသုံးပြုပေးတဲ့အတွက် အများကြီး ကျေးဇူးတင်ပါတယ်ရှင်။ 🙏",
            reply_markup=main_kb
        )
...
async def handle_review_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await send_typing(update, context)
    """User ပို့လိုက်သော Review စာသားကို Admin ထံ ပို့ဆောင်ပြီး ကျေးဇူးတင်စကားပြန်ဆိုခြင်း"""
    review_text = update.message.text
    user_id = update.effective_user.id
    
    # ၁။ Admin ထံသို့ ပေးပို့ခြင်း
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
    ...
    # ၂။ User ထံသို့ အောင်မြင်ကြောင်းနှင့် ကျေးဇူးတင်စကား ပြန်ပို့ခြင်း (kb ကို သုံးထားပါသည်)
    await update.message.reply_text(
        "လူကြီးမင်းရဲ့ Review ပေးပို့မှု အောင်မြင်သွားပါပြီရှင်။ ✅\n\nTarot မေးမြန်းပေးတဲ့အတွက်လည်း အများကြီး ကျေးဇူးတင်ပါတယ်ရှင်။ 🙏",
        reply_markup=main_kb 
    )
    return ConversationHandler.END
...
async def handle_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await send_typing(update, context)
    """User ပို့လိုက်သော အကြံပြုစာကို Admin ထံ ပို့ဆောင်ပြီး ကျေးဇူးတင်စကားပြန်ဆိုခြင်း"""
    suggestion_text = update.message.text
    user_id = update.effective_user.id
    
    # ၁။ Admin ထံသို့ ပို့ဆောင်ခြင်း
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"💡 *New Suggestion Alert!*\n\nUser: `{user_id}`\nContent: {suggestion_text}",
            parse_mode="Markdown"
        )
    except Exception as e:
        logger.error(f"Failed to send suggestion to admin: {e}")

    # ၂။ User ထံသို့ ကျေးဇူးတင်စကား ပြန်ပို့ခြင်း (kb ကို သုံးထားပါသည်)
    await update.message.reply_text(
        "လူကြီးမင်းရဲ့ အကြံပြုချက်ကို လက်ခံရရှိပါတယ်ရှင်။ ✅\n\nအကြံပြုစာ ပေးပို့ပေးတဲ့အတွက် အများကြီး ကျေးဇူးတင်ပါတယ်ရှင်။ 🙏",
        reply_markup=main_kb 
    )
    return ConversationHandler.END

# 💡 ၁။ ယခင် share_logic နေရာတွင် အောက်ပါ အပိုင်းဖြင့် အစားထိုးပါ 💡
async def share_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await check_ban_status(update, context): return ConversationHandler.END
    
    await send_typing(update, context)
    """Share ၁ ခါလျှင် ၁ ခု (တစ်နေ့ ၁၀ ခုအထိ)၊ Max Credit 12 ခု"""
    user_id = update.effective_user.id
    ...
    try:
        with conn.cursor() as c:
            c.execute("SELECT credits, last_share_date, daily_share_count FROM users WHERE user_id = %s", (user_id,))
            res = c.fetchone()
            
            current_credits = res[0] if res else 0
            last_share_date = res[1] if res else None
            daily_share_count = res[2] if res else 0
            
            # နောက်ရက်ကူးသွားလျှင် ယနေ့ share ထားမှုကို 0 ပြန်လုပ်မည်
            if last_share_date != today:
                daily_share_count = 0
            
            if current_credits >= 12:
                # Credit အများဆုံး 12 ခု ပြည့်နေလျှင် (Share ခွင့်ပြုသော်လည်း Credit မတိုးပါ)
                status_msg = "📢 *သင့်ထံတွင် အများဆုံးထားရှိနိုင်သော Credit (၁၂) ခု ပြည့်နေပါပြီ!*\n\nCredit အချို့ကို အရင်ဆုံး အသုံးပြုပြီးမှ ထပ်မံရှာဖွေပါ။\n\n(သို့သော် အောက်ပါလင့်ခ်မှတစ်ဆင့် သူငယ်ချင်းများထံ ဆက်လက် Share ပေးနိုင်ပါတယ်ရှင် 🙏)"
            elif daily_share_count >= 10:
                # တစ်နေ့တာ Share limit 10 ကြိမ် ပြည့်သွားလျှင် (Share ခွင့်ပြုသော်လည်း Credit မတိုးပါ)
                status_msg = "📢 *ယနေ့အတွက် Share လုပ်၍ရနိုင်သော အများဆုံး (၁၀) ကြိမ် ပြည့်သွားပါပြီ!*\n\nမနက်ဖြန်တွင် Credit များ ထပ်မံရယူနိုင်ပါမည်။\n\n(သို့သော် အောက်ပါလင့်ခ်မှတစ်ဆင့် သူငယ်ချင်းများထံ ဆက်လက် Share ပေးနိုင်ပါတယ်ရှင် 🙏)"
            else:
                # သတ်မှတ်ချက်များ ကိုက်ညီသဖြင့် 1 Credit ပေါင်းပေးမည်
                c.execute("UPDATE users SET credits = credits + 1, last_share_date = %s, daily_share_count = %s + 1 WHERE user_id = %s", (today, daily_share_count, user_id))
                conn.commit()
                current_credits += 1
                daily_share_count += 1
                status_msg = f"🎉 *ကျေးဇူးတင်ပါတယ်!*\n\nShare တဲ့အတွက် အခမဲ့ *1 Credit* ရရှိပါပြီ။\n📊 ယနေ့ Share ခဲ့မှု: ({daily_share_count}/10)\n💳 သင့်လက်ရှိ Credit: {current_credits}/12\n\n(သတိပြုရန် - ယခုရရှိသော Credit များသည် ယနေ့ညသန်းခေါင်တွင် ပျက်ပြယ်သွားမည်ဖြစ်သည်)"
                
    except Exception as e:
        logger.error(f"Share error: {e}")
        status_msg = "❌ စနစ်အမှားအယွင်း ဖြစ်ပေါ်နေပါသည်။"
    finally:
        db_mgr.put_conn(conn)
    ...
    await update.message.reply_text(
        status_msg + "\n\n👇 အောက်က ခလုတ်ကို နှိပ်ပြီး Share လိုက်ပါဦး။",
...
async def handle_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_typing(update, context)
    """'💡 အကြံပေးမည်' ခလုတ်နှိပ်လျှင် စာသားတောင်းခြင်း"""
    await update.message.reply_text(
        "လူကြီးမင်းရဲ့ အကြံပြုချက် ဒါမှမဟုတ် ဝေဖန်ချက်တွေကို ဒီနေရာမှာ အသေးစိတ် ရေးသားပေးပို့နိုင်ပါတယ်ရှင်။ 🙏",
        reply_markup=ReplyKeyboardRemove() # စာရိုက်ရလွယ်အောင် Keyboard ခေတ္တဖျောက်ခြင်း
    )
    return WAIT_SUGGESTION_TEXT

async def handle_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await send_typing(update, context)
    """User ပို့လိုက်သော အကြံပြုစာကို Admin ထံ ပို့ဆောင်ခြင်း"""
    suggestion_text = update.message.text
    user_id = update.effective_user.id
    ...
    # ၂။ User ထံသို့ ကျေးဇူးတင်စကား ပြန်ပို့ခြင်း
    await update.message.reply_text(
        "လူကြီးမင်းရဲ့ အကြံပြုချက်ကို လက်ခံရရှိပါတယ်ရှင်။ အကြံပြုစာ ပေးပို့ပေးတဲ့အတွက် အများကြီး ကျေးဇူးတင်ပါတယ်ရှင်။ 🙏",
        reply_markup=main_kb # ပင်မ Menu ၃ ခု ပြန်ဖော်ပေးခြင်း
    )
    return ConversationHandler.END
...
async def post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin မှ စာသား သို့မဟုတ် ပုံကို Broadcast လုပ်ရန် (အကြီး/အသေး ၂ မျိုးလုံး လက်ခံသည်)"""
    ...
    # Message ထဲမှာ /post (သို့) /Post ပါ/မပါ အရင်စစ်ဆေးပါမည်
    ...
    # /post နဲ့စတာ မဟုတ်ရင် ဘာမှမလုပ်ပါ
    if not is_post_command:
        return

    # /post သို့မဟုတ် /Post ဆိုတဲ့စာသားကို ဖျက်ထုတ်ခြင်း
    # အကြီးရော အသေးရော ဖြတ်ထုတ်နိုင်ရန် re (Regex) ကိုသုံးပါမည်
    ...
    for u in users:
        ...
        try:
            if has_photo:
                # ပုံနဲ့စာဆိုရင် (စာမပါရင်လည်း ရတယ်)
                await context.bot.send_photo(chat_id=user_id, photo=file_id, caption=content_text)
            else:
                # စာသက်သက်ဆိုရင်
                await context.bot.send_message(chat_id=user_id, text=content_text)
            
            success += 1
            await asyncio.sleep(0.1) # Limit ကျော်မှာစိုးလို့ delay ခံပါသည်
        except Exception as e:
            fail += 1
...
async def add_credit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin မှ User တစ်ဦးကို Credit လက်ဆောင်ပေးခြင်း"""
    ...
    try:
        ...
        conn = db_mgr.get_conn(); c = conn.cursor()
        c.execute("UPDATE users SET credits = credits + %s WHERE user_id = %s", (amount, target_id))
        conn.commit(); db_mgr.put_conn(conn)
        
        await update.message.reply_text(f"✅ User `{target_id}` ကို {amount} credits ဖြည့်သွင်းပေးလိုက်ပါပြီ။")
        await context.bot.send_message(chat_id=target_id, text=f"🎁 Admin က သင့်ကို {amount} credits လက်ဆောင်ပေးလိုက်ပါတယ်ရှင်။")
    except (IndexError, ValueError):
        await update.message.reply_text("❌ အသုံးပြုပုံ - `/add_credit USER_ID AMOUNT`")
...
async def check_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        target_id = int(context.args[0])
        conn = db_mgr.get_conn(); c = conn.cursor()
        c.execute("SELECT credits, birthday, zodiac, join_date FROM users WHERE user_id = %s", (target_id,))
...
async def send_morning_quote(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        try:
            await context.bot.send_message(
                chat_id=user[0], 
                text=f"{quote}\n\nယနေ့အတွက် လူကြီးမင်းရဲ့ လမ်းညွှန်ချက်ကို အခမဲ့ ကဒ်နှိပ်ပြီး ကြည့်လိုက်ပါဦးနော်။ ✨",
...
async def send_daily_stats_to_admin(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with conn.cursor() as c:
            ...
            # ၃။ ယနေ့ Share ခဲ့သော စုစုပေါင်း အကြိမ်အရေအတွက် (Virality Check)
            c.execute("SELECT SUM(daily_share_count) FROM users")
            total_shares_today = c.fetchone()[0] or 0
            
            # ၄။ ယနေ့ Share Limit (၁၀ ကြိမ်) ပြည့်သွားသူ အရေအတွက်
            c.execute("SELECT COUNT(*) FROM users WHERE daily_share_count >= 10")
            hardcore_users = c.fetchone()[0] or 0
        ...
        report_msg = (
            ...
            "--------------------------\n"
            f"🚀 ယနေ့ Share မှု စုစုပေါင်း: {total_shares_today} ကြိမ်\n"
            f"🔥 Share Limit ပြည့်သွားသူ: {hardcore_users} ဦး\n\n"
...
async def handle_daily_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Daily Card ခလုတ်နှိပ်လျှင် Random ကဒ်တစ်ကဒ်နှင့် General ဟောကိန်းပြခြင်း"""
    query = update.callback_query
    await query.answer()
    user_id = update.effective_user.id
    
    # Loading အနေနဲ့ Typing ပြခြင်း
    ...
    img_path = f"cards/{card_id}.jpg"
    caption = f"🌟 ယနေ့အတွက် လမ်းညွှန်ကဒ်: *{card_data.get('name')}*"
    
    if os.path.exists(img_path):
        await context.bot.send_photo(
            chat_id=user_id,
            photo=open(img_path, 'rb'),
            caption=f"{caption}\n\n📝 {card_data.get('general')}",
            parse_mode="Markdown"
        )
    else:
        await query.message.reply_text(f"{caption}\n\n📝 {card_data.get('general')}", parse_mode="Markdown")      

# 💡 ၂။ ယခင် reset_daily_credits နေရာတွင် အောက်ပါ အပိုင်းဖြင့် အစားထိုးပါ 💡
async def reset_daily_credits(context: ContextTypes.DEFAULT_TYPE):
    """ညသန်းခေါင်ရောက်ပါက User အားလုံး၏ Credit ကို ဖျက်ပြီး ၂ ခု ပြန်လည်ဖြည့်ပေးမည်"""
    conn = db_mgr.get_conn()
    try:
        with conn.cursor() as c:
            # User အားလုံး (Condition မလိုတော့ပါ) ရဲ့ Credit ကို 2 အဖြစ် အသေ သတ်မှတ်မည်
            # ၎င်းသည် 0 ချပြီး 2 ပြန်ပေါင်းခြင်းနှင့် ရလဒ်အတူတူပင် ဖြစ်ပါသည်
            c.execute("UPDATE users SET credits = 2, daily_share_count = 0")
        conn.commit()
        logger.info("✅ All Users' Credits Reset to 2 and Shares Restored to 0.")
    finally:
        db_mgr.put_conn(conn)
...
# 💡 အဆင့် (၁) - လူကြီးမင်းရဲ့ Bot Folder ထဲမှာ 'welcome.jpg' ဆိုတဲ့ ပုံတစ်ပုံ ရှိနေဖို့ လိုပါမယ်ရှင်။
# (ဒါမှမဟုတ် တခြားအမည်ပေးထားရင် အောက်က code ထဲမှာ အမည်လိုက်ပြောင်းပေးပါ)

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """နာမည်အပြည့်အစုံခေါ်ပြီး ပုံနှင့်အတူ ကြိုဆိုခြင်း"""
    user_id = update.effective_user.id
    full_name = update.effective_user.full_name # <--- နာမည်အပြည့်အစုံယူခြင်း
    
    await send_typing(update, context)
    

    
    # ကျွန်မတို့ အတည်ပြုထားတဲ့ စာသား
    welcome_text = (
...
# 🖼️ ပုံနှင့် စာသားတွဲပို့ခြင်း (ဒီအပိုင်းကိုပဲ အစားထိုးပါ)
    global WELCOME_PHOTO_ID
    photo_path = "cards/welcome.png" 
    
    if WELCOME_PHOTO_ID:
        # ✅ Telegram ထဲမှာ ရှိပြီးသားပုံကို file_id နဲ့ ချက်ချင်းပို့မည် (အရမ်းမြန်သည်)
        ...
        # ပထမဆုံးအကြိမ် တစ်ခါပဲ Upload တင်မည်
        ...
        WELCOME_PHOTO_ID = sent_msg.photo[-1].file_id # ID ကို မှတ်ထားလိုက်သည်
    else:
        await update.message.reply_text(welcome_text, reply_markup=main_kb, parse_mode="Markdown")
    
        # Database ထဲသို့ ထည့်ခြင်း (ယခင်အတိုင်း)
    conn = db_mgr.get_conn()
    with conn.cursor() as c:
...
async def handle_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "*၂။ ✨ နေ့စဉ်အခမဲ့ မေးမြန်းခြင်း*\n"
        "• လူကြီးမင်းအနေနဲ့ တစ်ရက်ကို (၂) ကြိမ် အခမဲ့ မေးမြန်းနိုင်ပါတယ်။\n"
        "• မေးမြန်းခွင့်များကို ညသန်းခေါင် (၁၂) နာရီမှာ အလိုအလျောက် ပြန်လည်ဖြည့်တင်းပေးမှာပါရှင်။\n\n"
        "*၃။ 📤 မေးမြန်းခွင့် ထပ်မံရယူနည်း*\n"
        "• တစ်ရက်တာ မေးမြန်းခွင့်ကုန်ဆုံးသွားရင် '📤 Share မည်' ခလုတ်ကို သုံးပြီး သူငယ်ချင်းတွေကို Bot အကြောင်း မျှဝေပေးရုံနဲ့ နောက်ထပ် (၁) ကြိမ် ထပ်မံမေးမြန်းခွင့် ရရှိမှာ ဖြစ်ပါတယ်ရှင်။\n\n"
        "*၄။ ✍️ အကြံပြုချက်ပေးခြင်း*\n"
        "• ထင်မြင်ချက်များရှိရင် '✍️ အကြံပြုမည်' ခလုတ်မှတစ်ဆင့် ပေးပို့နိုင်ပါတယ်ရှင်။\n\n"
...
def main():
    ...
    job_queue.run_daily(reset_daily_credits, time=time(hour=0, minute=0, second=0))
    job_queue.run_daily(send_morning_quote, time=time(hour=6, minute=0, second=0))
    ...
    love_conv_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^❤️ ဖူးစာစစ်မည်$"), love_start)],
        states={
    ...
    app.add_handler(MessageHandler(filters.Regex("^✍️ အကြံပြုမည်$"), handle_suggestion))

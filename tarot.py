...
def init_db():
    ...
    try:
        with conn.cursor() as c:
            # ၁။ Table မရှိရင် အရင်ဆောက်မည် (User 7,000 ရှိပြီးသားမို့ ရှိရင် ဘာမှမလုပ်ဘဲ ကျော်သွားမည်)
            c.execute('''CREATE TABLE IF NOT EXISTS users (user_id BIGINT PRIMARY KEY)''')
            
            # ၂။ Column အားလုံးကို Safe ဖြစ်အောင် တစ်ခုချင်းစီ စစ်ပြီးတိုးခြင်း [cite: 2026-04-22]
            # IF NOT EXISTS ကို သုံးထားလို့ မရှိသေးတာတွေပဲ တိုးသွားမှာပါ
            all_columns = [
                ...
                ("is_banned", "BOOLEAN DEFAULT FALSE") # 👈 ကျန်ခဲ့တဲ့ Column တွေအကုန် ထည့်ထားပါတယ်
            ]
            
            for col_name, col_type in all_columns:
                try:
                    # PostgreSQL ရဲ့ IF NOT EXISTS စနစ်ကို သုံးခြင်းဖြင့် Error မတက်တော့ပါ
                    c.execute(f"ALTER TABLE users ADD COLUMN IF NOT EXISTS {col_name} {col_type}")
                except Exception as e:
                    # တစ်ခုခုလွဲရင်တောင် ကျန်တာတွေကို ဆက်လုပ်ဖို့ rollback မလုပ်တော့ပါ [cite: 2026-04-22]
                    logger.info(f"Database Patch Note: {col_name} check done.")
            ...
            conn.commit() # အပြောင်းအလဲအားလုံးကို အတည်ပြုသိမ်းဆည်းမည်
            logger.info("✅ Database Patch Success! User 7,000 are safe.")
    except Exception as e:

...
async def share_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with conn.cursor() as c:
            # User မရှိသေးလျှင် အရင်ထည့်ပြီးမှ Credit တိုးပေးမည့် Logic (UPSERT)
            c.execute("""
                INSERT INTO users (user_id, credits, last_share_date) 
                VALUES (%s, 1, %s) 
                ON CONFLICT (user_id) 
                DO UPDATE SET credits = users.credits + 1, last_share_date = EXCLUDED.last_share_date
            """, (user_id, today))
        conn.commit()
    except Exception as e:

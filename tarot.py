...
def init_db():
    ...
    try:
        with conn.cursor() as c:
            # ၁။ အဟောင်းတွေကို အရှင်းဖျက်ပြီး အသစ်ပြန်ဆောက်ခိုင်းခြင်း (Data ပျက်ပြီးသားမို့လို့ အစကပြန်စတာ အကောင်းဆုံးပါ) [cite: 2026-04-22]
            # Table အဟောင်းတွေနဲ့ Column အမည်လွဲနေတာတွေကို ရှင်းပစ်ဖို့ ဖြစ်ပါတယ်
            c.execute("DROP TABLE IF EXISTS users CASCADE")
            
            # ၂။ ကုဒ်ထဲက လိုအပ်တဲ့ Column တွေအားလုံးနဲ့ Users Table ကို အသစ်ဆောက်ခြင်း
            ...
            # ၃။ အခြား လိုအပ်သော Table များကိုလည်း အသစ်ပြန်ဆောက်ခြင်း
            c.execute("CREATE TABLE IF NOT EXISTS feedbacks (id SERIAL PRIMARY KEY, user_id BIGINT, rating TEXT, comment TEXT, submitted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
            c.execute("CREATE TABLE IF NOT EXISTS broadcast_logs (id SERIAL PRIMARY KEY, user_id BIGINT, message_id BIGINT, batch_id TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
            
            conn.commit()
            logger.info("✅ Database Tables have been REBUILT correctly according to the code.")
    except Exception as e:

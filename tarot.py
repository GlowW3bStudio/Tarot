...
# ၁။ Credit ရှိမရှိ စစ်ဆေးခြင်း (Admin ဆိုလျှင် ကျော်မည်)
        if user_id != ADMIN_ID:
            if not res or res[0] is None or res[0] <= 0:
                ...
                # --- Share ခလုတ်အတွက် ပြင်ဆင်ခြင်း ---
                full_message = "ဒီ tarot botလေးသုံးကြည့် မိုက်တယ်\n@TarotBayDinBot"
                encoded_message = urllib.parse.quote(full_message)
                share_url = f"https://t.me/share/url?url=&text={encoded_message}"
                share_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🚀 သူငယ်ချင်းများထံ Share မည်", url=share_url)]])
                
                # --- စာပို့ခြင်း (msg ဟု မှတ်သားထားမည်) ---
                msg = await update.message.reply_text(
                    "❌ ယနေ့အတွက် အခမဲ့ မေးမြန်းခွင့် ကုန်ဆုံးသွားပါပြီရှင်။\n\nထပ်မံမေးမြန်းလိုပါက အောက်ပါခလုတ်ကိုနှိပ်၍ သူငယ်ချင်းများထံ မျှဝေပြီး မေးမြန်းခွင့်များ အခမဲ့ ရယူနိုင်ပါတယ်ရှင်။ 👇",
                    reply_markup=share_keyboard
                )
                
                # --- ၁၀ မိနစ် (စက္ကန့် ၆၀၀) အကြာတွင် ဖျက်ရန် Timer တပ်ခြင်း ---
...
# ၂။ Credit စစ်ဆေးခြင်း
    ...
    if not res or res[0] <= 0:
        ...
        context.job_queue.run_once(
            delete_old_message,
            600, 
            data={'chat_id': user.id, 'message_id': msg.message_id}
        )
...
# '✅ မှန်ကန်ပါသည်' ကို နှိပ်ပါက Credit စစ်မည်
    ...
    if not res or res[0] <= 0:
        ...
        # --- စာပို့ခြင်း (edit_message_text ဖြင့် စာဟောင်းကို ပြင်ရေးမည်) ---
        ...
        # --- ၁၀ မိနစ် (စက္ကန့် ၆၀၀) အကြာတွင် ဖျက်ရန် Timer တပ်ခြင်း ---
        # ဤနေရာတွင် query ထဲရှိ မူလ message_id ကို တိုက်ရိုက်ယူသုံးပါသည်
        context.job_queue.run_once(
            delete_old_message,
            600, 
...
async def handle_review_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await send_typing(update, context)
    """User ပို့လိုက်သော Review စာသားကို Admin ထံ Username/ID တွဲ၍ ပို့ပေးခြင်း"""
    ...
    # ၁။ Admin ထံသို့ Username ရော ID ရော တွဲ၍ ပေးပို့ခြင်း
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
    ...
    # ၂။ User ထံသို့ Admin ဆီ ပို့ပေးပြီးကြောင်း အတည်ပြုစာ ပြန်ပို့ခြင်း
    await update.message.reply_text(
...
# --- လိုင်း ၆၄၇ ဝန်းကျင်ရှိ handle_suggestion_text function ကို ဤကုဒ်ဖြင့် အစားထိုးပါ ---

async def handle_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await send_typing(update, context)
    """User ပို့လိုက်သော အကြံပြုစာကို Admin ထံ Username ပါဝင်အောင် ပို့ဆောင်ခြင်း"""
    suggestion_text = update.message.text
    user = update.effective_user
    user_id = user.id
    
    # Username ရှိလျှင် @ ဖြင့်ပြမည်၊ မရှိလျှင် ID သာပြမည်
    username_info = f"@{user.username}" if user.username else f"User ID: {user.id}"
    
    # ၁။ Admin ထံသို့ အသေးစိတ် ပို့ဆောင်ခြင်း
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"💡 *New Suggestion Alert!*\n\nUser: {username} (`{user_id}`)\nContent: {suggestion_text}",
    ...
    # ၂။ User ထံသို့ Admin ဆီ ပို့ပေးပြီးကြောင်းပါဝင်သော ချိုသာသည့်စာသား ပြန်ပို့ခြင်း
    await update.message.reply_text(
...
# 💡 ယခင် share_logic နေရာတွင် အောက်ပါ အပိုင်းဖြင့် အလုံးစုံ အစားထိုးပါ 💡
async def share_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await check_ban_status(update, context): return ConversationHandler.END
    
    await send_typing(update, context)
    """Credit မစစ်တော့ဘဲ မျှဝေရန် စာသားသက်သက်သာ ပြမည့် Share Logic"""
    user_id = update.effective_user.id
    ...
    try:
        with conn.cursor() as c:
            # Credit တိုးမည့် ကန့်သတ်ချက်များ (၁၀ ခါပြည့်၊ ၁၂ ခါပြည့်) အားလုံးကို ဖြုတ်လိုက်ပြီး
            # နှိပ်တိုင်း Credit ၁ ခု ပုံမှန်တိုးပေးရန်သာ ရေးထားပါသည် (မလိုလျှင် ဒီစာကြောင်းပါ ဖြုတ်နိုင်သည်)
            c.execute("UPDATE users SET credits = credits + 1, last_share_date = %s WHERE user_id = %s", (today, user_id))
        conn.commit()
    except Exception as e:
    ...
    # ရွေးချယ်ထားသော စာသားကိုသာ အသေ (Static) အနေဖြင့် အမြဲပြမည်
...
async def handle_daily_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    await query.answer()
        # ---> [အဆင့် ၃: ဤကုဒ်လေးကို အသစ်ပေါင်းထည့်ပါ] <---
    try:
        # ယနေ့အတွက် ကဒ်ရောက်နေပါပြီ ဆိုသော Notification Message ကို ဖျက်ပစ်ပါမည်
        await query.message.delete()
    except Exception:
        pass
    # ----------------------------------------
...
def main():
    ...
    job_queue.run_daily(reset_daily_credits, time=time(hour=17, minute=0, second=0))

...
async def handle_feedback_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "give_review":
        # ၁။ Review ပေးမည် နှိပ်လိုက်သောအခါ ပြမည့်စာသား
        await query.edit_message_text(
            "ဟုတ်ကဲ့ပါရှင်။ လူကြီးမင်းရဲ့ အတွေ့အကြုံလေးတွေကို သိချင်လို့ အောက်မှာ စာသားအတိုင်း ရိုက်ပို့ပေးပါဦးနော်။"
        )
...
async def handle_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Suggestion sent failed: {e}")

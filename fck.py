...
async def handle_permanent_bday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with conn.cursor() as c:
            # ဤနေရာတွင်သာ DB ထဲသို့ အမြဲတမ်း သိမ်းဆည်းမည်ဖြစ်သည်
            c.execute("UPDATE users SET birthday = %s, zodiac = %s WHERE user_id = %s", (text, zodiac, user_id))
        conn.commit()
...
async def tarot_init(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with conn.cursor() as c:
            ...
            res = c.fetchone()
            # DB မှာ မွေးနေ့ရှိပြီးသားသူ (ခလုတ်ဖြင့် မှတ်ထားသူ) ဆိုလျှင် ကျော်သွားမည်
...
async def admin_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "(_Command လေးတွေကို နှိပ်လိုက်ရုံနဲ့ Copy ကူးပြီးသား ဖြစ်သွားမှာပါရှင် ✨_)\n\n"
        "📢 /post [စာသား/ပုံ]\n"
        ...
        "User တစ်ဦးအား Credit လက်ဆောင်ပေးရန်။\n\n"
        "🧧 /giftall [Amount]\n"
        "User အားလုံးကို Credit လက်ဆောင် တစ်ပြိုင်နက်ပေးရန်။ (ဥပမာ: `/giftall 5`)\n\n"
        "🚫 /ban [USER_ID]\n"
    ...
    # Click-to-Copy အလုပ်လုပ်ရန် parse_mode ကို "Markdown" ဟု ပြောင်းလဲထားပါသည်

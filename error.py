...
async def check_user_credits(user_id):
    """Credit ရှိမရှိ စစ်ဆေးရန် (Check Only) - Admin ဆိုလျှင် အမြဲ True ပေးမည်"""
    if user_id == ADMIN_ID:
        return True
...
async def deduct_credit(user_id):
    """Credit ကို တကယ်နှုတ်ယူခြင်း (Admin ဆိုလျှင် မနှုတ်ပါ)"""
    if user_id == ADMIN_ID:
        return
    ...
    try:
        with conn.cursor() as c:
            # အရင်ဆုံး လက်ရှိ credit အခြေအနေကို ကြည့်မည်
            c.execute("SELECT gift_credits, daily_credits FROM users WHERE user_id = %s", (user_id,))
            res = c.fetchone()
            if res:
                gift, daily = res[0], res[1]
                # Priority: Gift Credit ကို အရင်နှုတ်မည်၊ ၎င်းကုန်မှ Daily ကို နှုတ်မည်
                if gift > 0:
...
async def add_credit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        conn = db_mgr.get_conn(); c = conn.cursor()
        c.execute("UPDATE users SET gift_credits = gift_credits + %s WHERE user_id = %s", (amount, target_id))

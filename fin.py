...
# Tarot ကဒ် ၇၈ ချပ်ရဲ့ File ID များကို မှတ်ထားရန်
card_file_cache = {}
...
def init_db():
    ...
    try:
        with conn.cursor() as c:
            # ၁။ Users Table မရှိလျှင် အခြေခံဆောက်မည် (DROP TABLE ကို ဖြုတ်လိုက်ပါပြီ) [cite: 2026-04-22]
            c.execute('''CREATE TABLE IF NOT EXISTS users (user_id BIGINT PRIMARY KEY)''')
            
            # ၂။ လိုအပ်သော Column များကို တစ်ခုချင်းစီ စစ်ဆေးပြီး တိုးပေးမည့် Safe Logic [cite: 2026-04-22]
            # IF NOT EXISTS ပါဝင်သောကြောင့် Column ရှိနှင့်ပြီးသားဆိုလျှင် ဒေတာမပျက်ဘဲ ကျော်သွားပါမည်
            columns = [
            ...
            for col_name, col_type in columns:
                try:
                    # PostgreSQL 9.6+ အတွက် ADD COLUMN IF NOT EXISTS ကို သုံးခြင်း [cite: 2026-04-22]
                    c.execute(f"ALTER TABLE users ADD COLUMN IF NOT EXISTS {col_name} {col_type}")
                except Exception:
                    pass # တစ်ခုခုလွဲလျှင်လည်း ကျန်တာများကို ဆက်လုပ်ရန်

            # ၃။ အခြား Table များ (Feedbacks & Logs)
            ...
            conn.commit()
            logger.info("✅ Database is now SAFE and PERSISTENT. Users won't be deleted again.")
    except Exception as e:
...
async def handle_permanent_bday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with conn.cursor() as c:
        # UPSERT Logic: User ID မရှိသေးရင် အသစ်ထည့်မယ်၊ ရှိပြီးသားဆိုရင် Update လုပ်မယ် [cite: 2026-04-22]
          c.execute("""
            INSERT INTO users (user_id, birthday, zodiac, bday_status) 
            VALUES (%s, %s, %s, TRUE) 
            ON CONFLICT (user_id) 
            DO UPDATE SET 
...
async def guide_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    # အဆင့် ၁: Credit ရှိမရှိ အရင်ဆုံးစစ်မည် (မရှိရင် ဝင်ခွင့်မပေးပါ)
    if not await check_user_credits(user_id):
        await update.message.reply_text(NO_CREDIT_TEXT)
        return ConversationHandler.END
...
async def handle_guide_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    choice = query.data
    context.user_data['guide_mode'] = choice 
    
    await send_typing(update, context)

    if choice == "guide_outcome":
        # ရလဒ်ကို နှိပ်လျှင် ရွေးစရာ ၄ ခု ထပ်ပြမည်
        keyboard = [
            [InlineKeyboardButton("🎓 စာမေးပွဲရလဒ်", callback_data="guide_sub_exam"),
             InlineKeyboardButton("💼 အင်တာဗျူးရလဒ်", callback_data="guide_sub_interview")],
            [InlineKeyboardButton("💰 အရောင်းအဝယ်ရလဒ်", callback_data="guide_sub_trade"),
             InlineKeyboardButton("✨ အခြား", callback_data="guide_sub_other")],
            [InlineKeyboardButton("❌ မမေးတော့ပါ", callback_data="cancel_tarot")]
        ]
        await query.edit_message_text(
            "ဘယ်ကိစ္စရဲ့ ရလဒ်ကို သိရှိလိုတာလဲရှင်? ✨",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return SELECT_OUTCOME_SUB
    else:
        # လမ်းညွှန်ချက်ကို နှိပ်လျှင် ဂဏန်းတောင်းသည့်ဆီ တန်းသွားမည်
        context.user_data['guide_sub_mode'] = "advice"
        await query.edit_message_text(
            "ရှေ့ဆက်လုပ်ဆောင်မယ့် အရာအတွက် လမ်းညွှန်ချက် ရယူနိုင်ဖို့ ၁ မှ ၇၈ ကြား ဂဏန်း (၁) ခု ကို ရိုက်ပို့ပေးပါဦးရှင်။"
        )
        return GET_GUIDE_NUM

async def handle_outcome_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Outcome အမျိုးအစားကို ရွေးချယ်ခြင်း"""
    query = update.callback_query
    await query.answer()
    sub_choice = query.data.replace("guide_sub_", "")
    context.user_data['guide_sub_mode'] = sub_choice
    
    await send_typing(update, context)
    await query.edit_message_text(
        "ကောင်းပါပြီရှင်။ ✨ လူကြီးမင်း စိတ်ထဲမှာ အာရုံပြုလိုက်တဲ့ ၁ မှ ၇၈ ကြား ဂဏန်း (၁) ခု ကို ရိုက်ပို့ပေးပါဦးရှင်။"
    )
    return GET_GUIDE_NUM
...
async def process_guide_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ဂဏန်းမှန်မှ Credit ၁ ခု နှုတ်မည် (Time-out error မဖြစ်အောင် ဤနေရာတွင် စစ်သည်)
    await deduct_credit(user_id)
    ...
    sub_mode = context.user_data.get('guide_sub_mode')
    ...
    # JSON မှ Key များအလိုက် စာသားဆွဲထုတ်ခြင်း
    if sub_mode == "advice":
        result_msg = card_info.get("advice", "စာသား ပြင်ဆင်နေဆဲဖြစ်ပါတယ်ရှင်။")
        header = "💡 **ရှေ့ဆက်လမ်းညွှန်ချက်**"
    else:
        # outcomes dic ထဲမှ sub_mode (exam, interview, trade, other) ကို ယူသည်
    ...
    # မှတ်ချက်စာသားကို mode အလိုက် ခွဲခြားခြင်း
    if sub_mode == "advice":
        note_text = (
            "*(မှတ်ချက် - ဤလမ်းညွှန်ချက်သည် လက်ရှိအခြေအနေအပေါ် အခြေခံထားသော အကြံပြုချက်သာ ဖြစ်ပါသည်။ "
            "လူကြီးမင်း၏ ရှေ့ဆက်ရမည့် ကြိုးစားအားထုတ်မှုနှင့် မှန်ကန်သော ဆုံးဖြတ်ချက်များကသာ "
            "ပိုမိုကောင်းမွန်သော အနာဂတ်ရလဒ်များကို ဖန်တီးပေးနိုင်မှာ ဖြစ်ပါတယ်ရှင်)*"
        )
    else:
        # Outcome (ရလဒ်) အတွက် မှတ်ချက်စာသားအသစ်
        note_text = (
            "*(မှတ်ချက် - ဤဟောကိန်းသည် လူကြီးမင်း လုပ်ဆောင်ခဲ့သော ကိစ္စရပ်အပေါ် "
            "လက်ရှိစွမ်းအင်များအရ ထွက်ပေါ်လာသော ရလဒ်သာ ဖြစ်ပါသည်။ ဖြစ်ပေါ်လာသမျှ အဖြေများကို "
            "တည်ငြိမ်စွာ လက်ခံပြီး ကောင်းမွန်သော ရှေ့ဆက်မှုများ ပြုလုပ်နိုင်ပါစေရှင်)*"
        )

    final_text = (f"{header}\n\n📝 {result_msg}\n\n✨ စွမ်းအင်အဆင့်: {energy}%\n\n{note_text}")
    
    await update.message.reply_text(final_text, parse_mode="Markdown", reply_markup=main_kb, write_timeout=60)
    await asyncio.sleep(1) # ခဏနားမည်
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="ဟောစာတမ်းလေးကို သဘောကျတယ်ဆိုရင် Review လေး ပေးခဲ့ပါဦးရှင်။ ✨",
        reply_markup=get_review_keyboard()
...
# ၁။ Love Start - Gender-neutral စာသားများဖြင့် စတင်ခြင်း
async def love_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "လူကြီးမင်းတို့နှစ်ဦးရဲ့ အချစ်ရေးကံကြမ္မာကို ဘယ်နည်းလမ်းနဲ့ စစ်ဆေးချင်ပါသလဲရှင်။ နှစ်သက်ရာနည်းလမ်းကို ရွေးချယ်ပေးပါရှင်။ ✨"
...
# ၂။ ပထမလူ၏ ရာသီခွင်ကို ရွေးချယ်ခြင်း
async def zodiac_male_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("ကျေးဇူးပြု၍ တွဲဖက် (Partner) ၏ ရာသီခွင်ကို ဆက်လက်ရွေးချယ်ပေးပါရှင်။ ✨", reply_markup=get_zodiac_keyboard("female"))
    return ZODIAC_FEMALE

# ၃။ ဒုတိယလူ၏ ရာသီခွင်ကို ရွေးချယ်ပြီး အက္ခရာစဉ်အလိုက် Key တည်ဆောက်ခြင်း
async def zodiac_female_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.data == "love_cancel": return await love_cancel_handler(update, context)
    elif query.data == "zlove_back_male":
        await query.edit_message_text("လူကြီးမင်းရဲ့ ရာသီခွင်ကို အောက်ပါစာရင်းမှ ရွေးချယ်ပေးပါရှင်။ ✨", reply_markup=get_zodiac_keyboard("male"))
        return ZODIAC_MALE
        
    sign = query.data.split('_')[1]
    first = context.user_data.get('first_zodiac')
    # 💡 Alphabetical sorting လုပ်ပြီး Key တည်ဆောက်ခြင်း (Aries_Taurus)
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔙 ပြန်ရွေးမည်", callback_data="zlove_back_male")],
        [InlineKeyboardButton("❌ မမေးတော့ပါ", callback_data="love_cancel")]
    ]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    return ZODIAC_CONFIRM

# ၄။ ဟောစာတမ်းထုတ်ပြန်ခြင်း (Role-based & Actionable Advice)
async def zodiac_confirm_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "zlove_confirm_yes":
        ...
        # 💡 ပြင်ဆင်ထားသော Key များဖြင့် ပြန်ခေါ်ခြင်း
        pair_key = context.user_data.get('zodiac_pair_key')
        display_name = context.user_data.get('display_pair')
        
        # 💡 Load လုပ်ထားသော Variable အမှန်ကို သုံးခြင်း
        result_data = ZODIAC_LOVE_DATA.get(pair_key) 
        
        if result_data:
            # ရှာတွေ့ပါက အသစ်ရေးထားသော JSON ထဲမှ အရှည်များထုတ်ပြမည်
            final_text = (
                f"💘 **{display_name} တွဲဖက်ညီမှု ဟောစာတမ်း**\n\n"
            ...
            # ဖိုင်နာမည်မှားနေပါက သို့မဟုတ် ရှာမတွေ့ပါက ပြမည့် အရန်စာသား
    ...
    # 💡 အောက်က အဟောင်းထဲကနေ Back နဲ့ Cancel ကိုပါ ပေါင်းထည့်ထားပါသည်
        ...
        # စကားလုံးကိုပါ Inclusive ဖြစ်အောင် ပြင်ထားသည်
        await query.edit_message_text("ကျေးဇူးပြု၍ ပထမလူ၏ ရာသီခွင်ကို ပြန်လည်ရွေးချယ်ပေးပါရှင် ✨", reply_markup=get_zodiac_keyboard("male"))
        return ZODIAC_MALE
...
async def handle_cat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cat_text == "cancel_tarot" or cat_text == "love_cancel":
...
async def handle_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ဒီအပိုင်းလေး ထပ်ဖြည့်ပါ ---
    if sub_text == "cancel_tarot" or sub_text == "love_cancel":
        return await cancel_tarot(update, context)
    # -------------------------
...
async def post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin ပို့လိုက်သော Message ကို Reply ပြန်၍ User အားလုံးထံ ဖြန့်ဝေခြင်း"""
    if update.effective_user.id != ADMIN_ID:
        return

    # Message ကို Reply ပြန်ထားခြင်း ရှိမရှိ စစ်ဆေးခြင်း
    if not update.message.reply_to_message:
        await update.message.reply_text("❌ ပို့ချင်တဲ့ Message (စာသား/ပုံ) ကို အရင်ပို့ပါ။ ပြီးမှ အဲ့ဒီစာကို Reply ပြန်ပြီး /post လို့ ရိုက်ပေးပါရှင်။")
        return
...
# User အားလုံး (Active ဖြစ်သူများသာ) ကို Database မှ ဆွဲထုတ်ခြင်း
    ...
    # အလုပ်ကို Background Task အဖြစ် လွှဲပြောင်းပေးလိုက်ခြင်း (No Delay)
    asyncio.create_task(do_broadcast(context, users, update.effective_chat.id, target_msg.message_id, batch_time, ADMIN_ID))
...
async def send_private(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin မှ User တစ်ဦးတည်းကို Message တစ်ခုအား Reply ပြန်၍ ပို့ဆောင်ခြင်း"""
    if update.effective_user.id != ADMIN_ID:
        return

    # User ID ပါမပါ နှင့် Reply လုပ်မထားခြင်း စစ်ဆေးခြင်း
    if not context.args or not update.message.reply_to_message:
        await update.message.reply_text("❌ အသုံးပြုပုံ - ပို့ချင်တဲ့စာကို Reply ပြန်ပြီး /send [USER_ID] လို့ ရိုက်ပါရှင်။")
        return
    ...
    try:
        ...
        # Premium Content ကို copy_message ဖြင့် ပို့ဆောင်ခြင်း
        ...
        await update.message.reply_text(f"✅ User {target_id} ထံသို့ပေးပို့ပြီးပါပြီ။")

    except ValueError:
        await update.message.reply_text("❌ User ID သည် ကိန်းဂဏန်း (Number) ဖြစ်ရပါမည်။")
    except Exception as e:
        await update.message.reply_text(f"❌ ပို့၍မရပါရှင်။ Error: {e}")
...
async def send_morning_quote(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        try:
            ...
            caption_text = f"{quote}\n\nဒီနေ့အတွက် လူကြီးမင်းကို လမ်းညွှန်ပေးမယ့် Tarot ကဒ်လေးက ဘာဖြစ်မလဲ? ကံဇာတာကို သိရှိနိုင်ဖို့ အောက်ကခလုတ်လေးကို နှိပ်ပြီး အခမဲ့ ကဒ်နှိုက်ကြည့်လိုက်ပါဦးနော်။ 🃏👇"
...
async def handle_daily_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Daily ကဒ်နှိုက်ခြင်း Logic (စာသားမပြင်ဘဲ Logic နှင့် Menu ပါဝင်အောင် ပြင်ဆင်ပြီး)"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        # ၃။ အောင်မြင်စွာ ပို့ပြီးမှသာ မနက်ခင်း Noti စာဟောင်းကို ဖျက်မည်
        try:
        ...
        logger.error(f"Error in handle_daily_card: {e}")
        await context.bot.send_message(
            chat_id=user_id,
            text="❌ နည်းပညာပိုင်း အခက်အခဲကြောင့် ကဒ်နှိုက်၍ မရသေးပါရှင်။ ခဏနေမှ ပြန်လည် ကြိုးစားပေးပါဦးနော်။",
            reply_markup=main_kb # Error တက်လျှင်လည်း Menu ပြန်ပြပေးမည်
        )
...
async def handle_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "🔮 *Tarot မေးမည်*\n"
        "အတိတ်၊ ပစ္စုပ္ပန်နဲ့ အနာဂတ် ကံကြမ္မာတွေကို ကဒ် (၃) ကဒ်စနစ်နဲ့ အသေးစိတ် တွက်ချက်ဟောကြားပေးမှာ ဖြစ်ပါတယ်ရှင်။\n\n"
        "🎯 *ကံကြမ္မာ လမ်းညွှန်*\n"
        "ကဒ် (၁) ကဒ်တည်းနဲ့ လက်ရှိကိစ္စတစ်ခုရဲ့ 'ရလဒ်' ဒါမှမဟုတ် ရှေ့ဆက်ရမယ့် 'လမ်းညွှန်ချက်' ကို တိတိကျကျ သိလိုတဲ့အခါ အသုံးပြုနိုင်ပါတယ်ရှင်။\n\n"
        "❤️ *ချစ်သူနဲ့ကိုက်ညီမှု(RS)*\n"
        "နှစ်ဦးသားရဲ့ တွဲဖက်ညီမှုကို Tarot ကဒ် ဒါမှမဟုတ် ရာသီခွင်နည်းလမ်းတွေနဲ့ စစ်ဆေးပေးသွားမှာပါရှင်။\n\n"
        ...
        "✍️ *အကြံပြုစာပို့ရန်*\n"
        "Botနဲ့ပတ်သက်တဲ့ ထင်မြင်ချက်တွေ ဒါမှမဟုတ် လိုအပ်ချက်တွေကို Admin ထံ တိုက်ရိုက် ပေးပို့နိုင်ပါတယ်ရှင်။\n\n"
        "📤 *Share မည်*\n"
        "ဒီBotလေးကို အသုံးပြုရတာ သဘောကျလို့ သူငယ်ချင်းတွေကိုလည်း မျှဝေပေးချင်တယ်ဆိုရင် ဒီခလုတ်လေးကနေတစ်ဆင့် ပြန်လည်မျှဝေပေးနိုင်ပါတယ်ရှင်။\n\n"
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            SELECT_OUTCOME_SUB: [CallbackQueryHandler(handle_outcome_sub, pattern="^guide_sub_")],

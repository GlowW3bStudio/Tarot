...
async def process_reading(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, card_id in enumerate(selected):
        ...
        # --- 💡 File ID Cache Logic ---
        # အရင်က ပို့ဖူးလို့ ID ရှိပြီးသားဆိုရင် ID ကို သုံးမယ်
        if card_id in card_file_cache:
            photo_to_send = card_file_cache[card_id]
        elif os.path.exists(img_path):
            # မရှိသေးရင် အသစ်တင်ပြီး ID ကို မှတ်ထားမယ်
    ...
    if media_group:
        try:
            ...
            # --- 💡 အသစ်တင်လိုက်တဲ့ ပုံတွေရဲ့ File ID ကို သိမ်းထားခြင်း ---
...
async def handle_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Markdown Error မတက်အောင် parse_mode ကို ဖယ်လိုက်ပါသည်
        await context.bot.send_message(
            chat_id=ADMIN_ID, 
...
# ၆။ Conversation Handlers များ
    ...
    love_conv_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^❤️ ချစ်သူနဲ့ကိုက်ညီမှု\(RS\)$"), love_start)], # Regex အမှန်
        states={
    ...
    # ၇။ App ထဲသို့ Handlers အားလုံး ထည့်သွင်းခြင်း (Admin Commands ကို အပေါ်တင်ပါ)
    app.add_handler(CommandHandler("help", admin_help)) # Admin အတွက် /help
    app.add_handler(CommandHandler("start", start))     # User အတွက် /start

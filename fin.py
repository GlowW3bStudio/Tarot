...
# ၆။ Conversation Handlers များ
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            WAIT_PERMANENT_BDAY: [ # 🎂 မွေးနေ့မှတ်တဲ့ State
                CallbackQueryHandler(bday_manage_callback),
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_permanent_bday)],
            WAIT_ZODIAC_PERS: [CallbackQueryHandler(handle_zodiac_choice)],
            WAIT_GENDER_PERS: [CallbackQueryHandler(handle_gender_personality)]
        },
        fallbacks=[
    ...
    love_conv_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"^❤️ ချစ်သူနဲ့ကိုု\(RS\)$"), love_start)],
        states={

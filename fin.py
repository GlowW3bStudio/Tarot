...
async def zodiac_confirm_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "zlove_confirm_yes":
        ...
        result_data = Z_LOVE_DATA.get(pair_key) # 💡 JSON key အသစ်ကို သုံးမည်
        
        if not result_data:
            result_data = {"percent": "??%", "strength": "ပြင်ဆင်နေဆဲ", "weakness": "ပြင်ဆင်နေဆဲ", "advice": "ပြင်ဆင်နေဆဲ"}

        final_text = (
            f"💘 **{display_name} တွဲဖက်ညီမှု ဟောစာတမ်း**\n\n"
            f"💖 ကိုက်ညီမှု: {result_data['percent']}\n\n"
            f"💪 အားသာချက်: \n{result_data['strength']}\n\n"
            f"⚠️ သတိထားရန်: \n{result_data['weakness']}\n\n"
            f"💡 အကြံပြုချက်: \n{result_data['advice']}\n\n"
            f"✨ လူကြီးမင်းတို့ချစ်သူနှစ်ဦး ပျော်ရွှင်ချမ်းမြေ့ပါစေရှင်။ ✨"
        )

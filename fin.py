...
async def process_guide_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၁။ ဂဏန်း Validation စစ်ဆေးခြင်း
    ...
    # ၂။ Random Shuffle Logic
    all_cards = [str(i) for i in range(1, 79)]
    random.shuffle(all_cards) 
    selected_card_id = all_cards[int(num_text) - 1] 
    
    # ၃။ Data နှင့် ပုံလမ်းကြောင်း ပြင်ဆင်ခြင်း
    mode = context.user_data.get('guide_mode')
    card_info = GUIDE_DATA.get(selected_card_id, {})
    card_name = TAROT_DATA.get(selected_card_id, {}).get("name", f"Card {selected_card_id}")
    img_path = f"cards/{selected_card_id}.jpg" # Image Folder ထဲရှိ ပုံလမ်းကြောင်း
    ...
    await asyncio.sleep(1) # စာသားမပို့ခင် အနည်းငယ်စောင့်ခြင်း

    # ၅။ ဟောစာတမ်းနှင့် Psychology Note ပို့ခြင်း
    note_text = (

...
main_kb = ReplyKeyboardMarkup([
    [KeyboardButton("🔮 Tarot မေးမည်")],
    [KeyboardButton("♈ ရာသီခွင်ပင်ကိုယ်စရိုက်"), KeyboardButton("❤️ ချစ်သူနဲ့ကိုက်ညီမှု(RS)")],
    [KeyboardButton("🎂 မွေးနေ့မှတ်မည်"), KeyboardButton("✍️ အကြံပြုစာပို့ရန်")],
    [KeyboardButton("📤 Share မည်"), KeyboardButton("❓ အသုံးပြုနည်း")]
...
WAIT_BDAY, WAIT_CAT, WAIT_SUB, WAIT_NUMS, WAIT_FEEDBACK, WAIT_REVIEW_TEXT, WAIT_SUGGESTION_TEXT, WAIT_PERMANENT_BDAY, WAIT_ZODIAC_PERS, WAIT_GENDER_PERS = range(10)
...
async def bday_manage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "delete_bday_data":
        ...
        try:
            with conn.cursor() as c:
                c.execute("UPDATE users SET birthday = '', zodiac = '' WHERE user_id = %s", (user_id,))
            conn.commit()
...
async def handle_permanent_bday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # (၆) Database တွင် မွေးနေ့ရော Zodiac ပါ အမြဲတမ်းသိမ်းဆည်းခြင်း
    conn = db_mgr.get_conn()
    try:
        with conn.cursor() as c:
            c.execute(
...
async def tarot_init(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with conn.cursor() as c:
            c.execute("SELECT birthday, zodiac FROM users WHERE user_id = %s", (user_id,))
            res = c.fetchone()
            # မွေးနေ့ရော Zodiac ပါ ရှိပြီးသားဖြစ်ပါက Category ဆီ တန်းသွားမည်
...
async def process_reading(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ဟောစာတမ်းတွက်ချက်ခြင်းနှင့် ပုံများပို့ခြင်း Logic (မူလအတိုင်း ၁၀၀% တူညီပါသည်)"""
    user_id = update.effective_user.id
    first_name = update.effective_user.first_name
    text = update.message.text
    
    # ၁။ ဂဏန်း ၃ ခု Validation စစ်ဆေးခြင်း (မူလ Logic အတိုင်း)
    try:
        ...
        if len(nums) != 3 or any(n < 1 or n > 78 for n in nums):
            raise ValueError
        if len(set(nums)) != 3:
            await update.message.reply_text("❌ ကဒ်တစ်ခုတည်းကို ထပ်၍ ရွေးချယ်၍မရပါရှင်။ မတူညီသော ဂဏန်း ၃ ခုကို ပြန်ရိုက်ပေးပါရှင်။ 🙏")
            return WAIT_NUMS
    except:
        await update.message.reply_text("❌ ကျေးဇူးပြု၍ ၁ မှ ၇၈ အကြား ဂဏန်း ၃ ခုကို မှန်ကန်စွာ ရိုက်ပေးပါရှင်။")
        return WAIT_NUMS

    # ၂။ Loading Message ပို့ခြင်း
    loading_msg = await update.message.reply_text(
        "⏳ လူကြီးမင်းအတွက် ဟောစာတမ်းကို တွက်ချက်နေပါတယ်ရှင်...",
        write_timeout=60, read_timeout=60
    )

    # ✨ ပုံတွေမဖတ်ခင် Typing Status ကို စတင်ပြသပါမည်
    await send_typing(update, context)

    # ၃။ Data နှင့် Zodiac ပြင်ဆင်ခြင်း
    ...
    for i, card_id in enumerate(selected):
        ...
        if card_id in card_file_cache:
            photo_to_send = card_file_cache[card_id]
        elif os.path.exists(img_path):
            photo_to_send = open(img_path, 'rb')
        else: continue
        media_group.append(InputMediaPhoto(photo_to_send, caption=caption_text if i == 0 else ""))
    
    if media_group:
        try:
            # ✨ ပုံတွေမပို့ခင်မှာ Typing Status တစ်ခါထပ်ပြပါမည်
            await send_typing(update, context)
            msgs = await update.message.reply_media_group(media=media_group, write_timeout=60, read_timeout=60)
            
            for i, m in enumerate(msgs):
                c_id = selected[i]
                if c_id not in card_file_cache:
                    card_file_cache[c_id] = m.photo[-1].file_id
        except Exception as e:
            logger.error(f"⚠️ Image Error: {e}")

    # ၆။ ၂ စက္ကန့် စောင့်စဉ်အတွင်း Typing ပြခြင်း
    await asyncio.sleep(2)
    await send_typing(update, context) # ✨ ဟောစာတမ်းစာသားမပို့ခင် Typing ပြန်ပြခြင်း

    # ၇။ ဟောစာတမ်းစာသား ပေါင်းစပ်ခြင်း (မူလ စာသားများအတိုင်း)
    timeframes = ["အတိတ် (သို့မဟုတ်) အကြောင်းရင်းခံ", "ပစ္စုပန် (သို့မဟုတ်) လက်ရှိအခြေအနေ", "အနာဂတ် (သို့မဟုတ်) ရှေ့ဆက်ဖြစ်လာနိုင်ခြေ"]
    final_report = f"🔮 *{sub_text} ဟောစာတမ်း*\n\n"
    
    for i, card_id in enumerate(selected):
        card_data = TAROT_DATA.get(card_id, {})
        card_name = card_data.get("name", f"Card {card_id}")
        meaning = card_data.get("general", "ပြင်ဆင်နေဆဲ") if main_key == "general" else card_data.get(main_key, {}).get(sub_key, "ပြင်ဆင်နေဆဲ")
        final_report += f"📅 *{timeframes[i]}*\n🃏 *ကဒ် {i+1}: {card_name}*\n📝 {meaning}\n\n"

        if i == 2 and main_key != "general":
            advice = card_data.get("zodiac_rules", {}).get(user_zodiac, "")
            if advice:
                final_report += f"🌟 *{user_zodiac} ရာသီဖွားများအတွက် အထူးအကြံပြုချက်:*\n{advice}\n\n"

    # ၈။ Credit နှုတ်ခြင်းနှင့် ဟောစာတမ်းပို့ခြင်း
    await deduct_credit(user_id)
    try:
    ...
    await update.message.reply_text("Review ပေးလိုပါသလားရှင်?", reply_markup=keyboard)
...
async def handle_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ၁။ Menu ခလုတ်များ နှိပ်မိပါက တားဆီးရန် ---
    menu_map = {
        "🔮 Tarot မေးမည်": tarot_init, "♈ ရာသီခွင်ပင်ကိုယ်စရိုက်": zodiac_personality_start,
        "❤️ ချစ်သူနဲ့ကိုက်ညီမှု(RS)": love_start, "🎂 မွေးနေ့မှတ်မည်": manage_bday,
        "✍️ အကြံပြုစာပို့ရန်": handle_suggestion, "📤 Share မည်": share_logic, "❓ အသုံးပြုနည်း": handle_help
    }
    if suggestion_text in menu_map:
        return await menu_map[suggestion_text](update, context)

    user = update.effective_user
    username_info = f"@{user.username}" if user.username else f"User ID: {user.id}"
    
    try:
        # ⚠️ အရေးကြီး - parse_mode ကို လုံးဝ ဖြုတ်ထားရပါမည်
        await context.bot.send_message(
            chat_id=ADMIN_ID, 
...
async def admin_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin သီးသန့် Help Command (မူလ Logic မပျက်စေဘဲ ပိုမိုဘေးကင်းအောင် ပြင်ဆင်ထားပါသည်)"""
    user_id = update.effective_user.id
    
    # Thura ရဲ့ ID နဲ့ ကိုက်ညီမှု ရှိ၊ မရှိ စစ်ဆေးခြင်း
    if user_id != ADMIN_ID:
        return
...
# ၆။ Conversation Handlers များ
    conv_handler = ConversationHandler(
        ...
        allow_reentry=True,
        per_message=False # <--- Text Input တွေ အလုပ်လုပ်ဖို့ False ထားရပါမယ်
    )
    ...
    love_conv_handler = ConversationHandler(
        ...
        allow_reentry=True,
        per_message=False # <--- Tarot Love မှာလည်း ဂဏန်းရိုက်ရမှာမို့လို့ False ထားပါ
    )

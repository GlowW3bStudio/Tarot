...
async def process_guide_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၁။ ဂဏန်း (၁ မှ ၇၈) ဟုတ်မဟုတ် အရင်စစ်ဆေးခြင်း
    ...
    # ၂။ စစ်မှန်သော Random စနစ်ဖြစ်ရန် ကတ်များကို အရင်နှောလိုက်ခြင်း (Shuffle Logic)
    all_cards = [str(i) for i in range(1, 79)]
    random.shuffle(all_cards) 
    
    # User ရိုက်လိုက်သော ဂဏန်းနေရာရှိ ကတ် ID ကို ဆွဲထုတ်ခြင်း
    selected_card_id = all_cards[int(num_text) - 1] 
    
    # ၃။ Data ဆွဲယူခြင်းနှင့် Header သတ်မှတ်ခြင်း
    ...
    energy = card_info.get("energy_score", random.randint(70, 98))
    await asyncio.sleep(2) # Reality Delay (၂ စက္ကန့် စောင့်ခြင်း)

    # ၄။ Final Note (Bot ၏ Authority ကို မြှင့်တင်ပေးသော Mind Control Logic)
    note_text = (
    ...
    final_text = (
        f"{header}\n\n"
        f"📝 {result_msg}\n\n"
        f"✨ စွမ်းအင်အဆင့်: {energy}%\n\n"
        f"{note_text}"
    )
    
    await wait_msg.delete()
    
    # ၅။ Timeout 60 ဖြင့် အဖြေလွှာ ပို့ပေးခြင်း
    await update.message.reply_text(
        final_text, 
        parse_mode="Markdown", 
...
async def handle_review_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Menu ခလုတ်များ Logic
    menu_map = {
        "🔮 Tarot မေးမည်": tarot_init, "🎯 ကံကြမ္မာ လမ်းညွှန်": guide_start,
        "❤️ ချစ်သူနဲ့ကိုက်ညီမှု(RS)": love_start, "♈ ရာသီခွင်ပင်ကိုယ်စရိုက်": zodiac_personality_start,
        "🎂 မွေးနေ့မှတ်မည်": manage_bday, "❓ အသုံးပြုနည်း": handle_help,
        "✍️ အကြံပြုစာပို့ရန်": handle_suggestion, "📤 Share မည်": share_logic
    }
    ...
    try:
        ...
        # [ ] များ ဖယ်ရှားပြီး စာသားကို တစ်ဆက်တည်း ပြင်ဆင်ခြင်း
        await context.bot.send_message(
            chat_id=ADMIN_ID,
    ...
    # User စာကိုဖျက်ပြီး Bot စာကို ၁ မိနစ်အကြာတွင် ဖျက်မည့် Logic
